
library(tibble)



Sujeito <- c(
  "O papa comunista",
  "A ONU",
  "O globalismo",
  "A rede Globo",
  "Luciano Huck",
  "Ciro Gomes",
  "Luladrão",
  "Lula",
  "Dilmanta",
  "O MEC",
  "O Congresso",
  "O Senado",
  "O STF",
  "Geraldo Alckmin",
  "Dória",
  "Rodrigo Maia",
  "Alexandre Frota",
  "O partido comunista da china",
  "Barack Obama",
  "Angela Merkel",
  "Enya", 
  "A UFRJ",
  "O PSOL",
  "A elite globalista",
  "A extrema-imprensa",
  "A grande mídia",
  "Pablo Vittar",
  "Xi Jinping",
  "Gilmar Mendes",
  "Establishment político"
)


Objeto <- c(
  "epidemia do vírus chinês",
  "a banheira do gugu",
  "a crise de valores",
  "o projeto de precarização",
  "a censura que sofri do Youtube",
  "a arma perfeita para justificar o fim das liberdades individuais e produzir concentração de poder como nunca",
  "protesto contra governadora do Michigan",
  "as falhas de segurança no laboratório de Wuhan",
  "implosão das contas públicas",
  "o próximo golpe que a esquerda americana quer dar nos EUA",
  "monitoramento de pessoas por celular",
  "a liminar de Lewandowski"
  )


projeto <- c(
  "o projeto de renda mínima global",
  "a mamadeira de piroca",
  "a doutrinação nas escolas",
  "o vírus chinês",
  "o partido comunista da china",
  "a corrupção",
  "o fascista Dória",
  "a destruição da família",
  "a concentração de poder como nunca",
  "a oportunidade de implementação de um modelo político chinês no mundo",
  "a política genocida do aborto",
  "a legalização do aborto",
  "ocultação dos grandes crimes",
  "os meios de opressão totalitária",
  "organizações genocidas como a OMS",
  " as intituições contra a soberania popular"
  )